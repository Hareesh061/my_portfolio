...
const toggle = () =>{
  ...
  return (
    <div>
        <nav className='fixed w-full z-20 top-0 bg-blue-300 bg-opacity-80'>
          ...
          <img src="https://logos.textgiraffe.com/logos/logo-name/37167579-designstyle-flippy-m.png" className='h-8 ml-4' alt="logo"/>
           ...
           <div className='flex md:order-2'>
             <button onClick={toggle} className='inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden focus:ring-2 focus:ring-gray-200'>
                <AiOutlineMenuUnfold className='text-blue-500 text-lg'>
...
<div className={`items-center justify-between w-full md:flex  md:w-auto md:order-1 ${isOpen? 'block' : 'hidden'}`}>
    <ul className={`flex flex-col p-4 md:p-0 mt-4 text-xl border cursor-pointer border-gray-100 rounded-lg md:flex-row md: space-x-8 md:mt-0 md:border-0 ${isOpen ? 'bg-blue-200 bg-opacity-70' : '' } `}>
    ...
    <li>
        <div className='block py-2 pl-3 pr-4  rounded md:p-0 hover:text-green-600  text-violet-700'>Contacts</div>
    </li>  </Link>
